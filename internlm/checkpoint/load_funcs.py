...
def load_hf_llama_pretrained_weights(folder, model):
    ...
    local_rank = int(local_rank) % 8
    print("local_rank", local_rank)
    # if local_rank >= 4:
    #     print(f"rank {rank} sleep  60s to avoid OOM", flush=True)
    #     import time
    #     time.sleep(60)
    ...
    for r in range(8):
        if r == local_rank:
            print(f"local_rank{local_rank} is loading model\n", end="", flush=True)
            # print(f"rank {rank} loading hf llama", flush=True)
            # exit()
            assert (
                folder is not None
            ), "Please specify the folder of the pretrained model"
            if gpc.is_rank_for_log():
                logger.info(f"Loading pretrained model from {folder}")
            ...
            for idx, i in enumerate(range(model.first_layer, model.last_layer)):
                if gpc.config.model_type == "LLAMA2":
                    ...
                    if not deep_split or (i + 2) % 2 == 0:
                        states[f"layers.{i}.attention.wq.weight"] = torch.chunk(
                            states.pop(
                                f"model.layers.{layer_ids}.self_attn.q_proj.weight"
                            ),
                        ...
                        states[f"layers.{i}.attention.wk.weight"] = torch.chunk(
                            states.pop(
                                f"model.layers.{layer_ids}.self_attn.k_proj.weight"
                            ),
                            gpc.get_world_size(ParallelMode.TENSOR),
                            dim=0,
                        )[gpc.get_local_rank(ParallelMode.TENSOR)]
                        states[f"layers.{i}.attention.wv.weight"] = torch.chunk(
                            states.pop(
                                f"model.layers.{layer_ids}.self_attn.v_proj.weight"
                            ),
                            gpc.get_world_size(ParallelMode.TENSOR),
                            dim=0,
                        )[gpc.get_local_rank(ParallelMode.TENSOR)]
                        states[f"layers.{i}.attention.wo.weight"] = torch.chunk(
                            states.pop(
                                f"model.layers.{layer_ids}.self_attn.o_proj.weight"
                            ),
                            gpc.get_world_size(ParallelMode.TENSOR),
                            dim=1,
                        )[gpc.get_local_rank(ParallelMode.TENSOR)]
                    ...
                    if not deep_split or (i + 2) % 2 == 1:
                        states[f"layers.{i}.feed_forward.w1.weight"] = torch.chunk(
                            states.pop(
                                f"model.layers.{layer_ids}.mlp.gate_proj.weight"
                            ),
                            gpc.get_world_size(ParallelMode.TENSOR),
                            dim=0,
                        )[gpc.get_local_rank(ParallelMode.TENSOR)]
                        ...
                        states[f"layers.{i}.feed_forward.w2.weight"] = torch.chunk(
                            states.pop(
                                f"model.layers.{layer_ids}.mlp.down_proj.weight"
                            ),
                            gpc.get_world_size(ParallelMode.TENSOR),
                            dim=1,
                        )[gpc.get_local_rank(ParallelMode.TENSOR)]
                    ...
                    if (
                        f"model.layers.{layer_ids}.self_attn.rotary_emb.inv_freq"
                        in states
                    ):
                        states.pop(
                            f"model.layers.{layer_ids}.self_attn.rotary_emb.inv_freq"
                ...
                for name in list(states.keys()):
                    if name.startswith(f"layers.{i}"):
                        current_states[name.replace(f".{i}.", f".{idx}.")] = states.pop(
                            name
                        )
            ...
            if (
                "tok_embeddings.weight" in model_state_keys
                or "tok_embeddings.word_embeddings.weight" in model_state_keys
            ):
                if gpc.config.model.get("embed_split_hidden", True):
                    current_states["tok_embeddings.weight"] = torch.chunk(
                        states["model.embed_tokens.weight"],
                        gpc.get_world_size(ParallelMode.TENSOR),
                        dim=1,
                    )[gpc.get_local_rank(ParallelMode.TENSOR)]
                else:
                    current_states["tok_embeddings.word_embeddings.weight"] = (
                        torch.chunk(
                            states["model.embed_tokens.weight"],
                            gpc.get_world_size(ParallelMode.TENSOR),
                            dim=1,
                ...
                assert (
                    model.first_layer == 0
                ), f"Expect model.first_layer to be 0, but got {model.first_layer}"

            if "output.weight" in model_state_keys:
                current_states["norm.weight"] = states["model.norm.weight"]
                current_states["output.weight"] = torch.chunk(
                    states["lm_head.weight"],
                    gpc.get_world_size(ParallelMode.TENSOR),
                    dim=0,
                )[gpc.get_local_rank(ParallelMode.TENSOR)]

            missing_keys, unexpected_keys = model.load_state_dict(
                current_states, strict=False
            )
            ...
            if gpc.get_local_rank(ParallelMode.DATA) == 0:
                pp_rank = (
                    ...
                    else gpc.get_local_rank(ParallelMode.PIPELINE)
                )
                logger.info(
                    f"Missing keys:{missing_keys}, unexpected keys:{unexpected_keys} in "
                    f"tp:{gpc.get_local_rank(ParallelMode.TENSOR)}, pp:{pp_rank}"
            ...
            print(f"used percent before empty_cache{psutil.virtual_memory().percent}\n",end="",flush=True)
            internlm_accelerator.empty_cache()
            del states
            del current_states
            print(f"used percent after empty_cache{psutil.virtual_memory().percent}\n",end="",flush=True)
            print(f"free memory :{psutil.virtual_memory().free}\n",end="",flush=True)
